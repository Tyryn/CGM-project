...
default_df <- read.csv("Tyryn Carnegie 20.05.2021.csv", header=FALSE)
...
ui <- dashboardPage(
  dashboardHeader(title = "Medtronic CGM explorer"),
  dashboardSidebar(
    ...
    fileInput(
      "infile",
      "Upload Carelink csv files (multiple possible, mmol/L or mg/dL)",
      multiple = TRUE,
      accept = c("text/csv",
  ...
  dashboardBody(
    tabsetPanel(
      ...
      # tabPanel("Impact of meals", value = "G",
      #          box(
      #            title = "Average glucose after meals",
      #            solidHeader = TRUE,
      #            status = "primary",
      #            width = 12,
      #            plotlyOutput("average_meal_elapse")
      #          ),
      #          box(
      #            title = "Average glucose after meals - 05:00-10:00",
      #            solidHeader = TRUE,
      #            status = "primary",
      #            width = 12
      #          ),
      #          box(
      #            title = "Average glucose after meals - 12:00-15:00",
      #            solidHeader = TRUE,
      #            status = "primary",
      #            width = 12
      #          ),
      #          box(
      #            title = "Average glucose after meals - 18:00-22:00",
      #            solidHeader = TRUE,
      #            status = "primary",
      #            width = 12
      #          ))
...
server <- function(session, input, output) {
  ...
  observe({
    shinyjs::toggle(id = "date1", condition = {
      "A" %in% input$tabs ||
        "C" %in% input$tabs ||
        "D" %in% input$tabs ||
        "E" %in% input$tabs || "F" %in% input$tabs || "G" %in% input$tabs
    })
    shinyjs::toggle(id = "date2", condition = {
      "A" %in% input$tabs ||
        "C" %in% input$tabs ||
        "D" %in% input$tabs || "E" %in% input$tabs || "G" %in% input$tabs
    })
    shinyjs::toggle(id = "target_range", condition = {
      "A" %in% input$tabs ||
        "B" %in% input$tabs ||
        "C" %in% input$tabs || "D" %in% input$tabs
    })
    ...
    shinyjs::toggle(id = "horizontal_rule4", condition = {
      "A" %in% input$tabs || "C" %in% input$tabs || "D" %in% input$tabs ||
        "E" %in% input$tabs || "G" %in% input$tabs
    })
  })
  ...
  getData <- reactive({
    ...
    if (is.null(inFile)) {
      ...
      csv_meal_exercise <- csv
      names(csv_meal_exercise) <-
        csv_meal_exercise %>% slice(5) %>% unlist()
      
      csv_meal_exercise <- csv_meal_exercise %>%
        select(Date, Time, `Event Marker`) %>%
        filter(grepl('Exercise|Meal', `Event Marker`)) %>%
      ...
      csv <- csv %>%
        dplyr::bind_rows(csv_meal_exercise) %>%
  ...
  cgmData <- reactive({
    cgmData <- getData() %>%
      ...
      group_by(date) %>%
      add_tally() %>%
      filter(n > 180) %>% # Quite a sharp limit on a days minimum sample size - possibly convert rows to NA
  ...
  date1Data <- reactive({
    date1_data <- cgmData() %>%
      ...
      ungroup() %>%
      # # Create weights, where days are weighted by the number of observations on that day 
      # mutate(day_weight = n/sum(n, na.rm = TRUE)) %>% 
      # # Count the number of days with over 70% in range
      # mutate(dummy_70 = as.numeric(percent_in_range >= 0.70)) %>% 
      # mutate(count_70 = sum(dummy_70*day_weight)) %>%
      # # Average daily low and high events
      # mutate(low_events_day_average = round(weighted.mean(low_events_day, n, na.rm = TRUE), 1)) %>%
      # mutate(high_events_day_average = round(weighted.mean(high_events_day, n, na.rm = TRUE), 1)) %>% 
      # mutate(n = n()) 
      mutate(n = n()) %>%
      # Count the number of days with over 70% in range
  ...
  output$best_day_1 <- renderText({
    ...
    best_day_percent <-
      paste0(best_day_percent$day_of_week[1],
             ":\n ",
             paste0(round(
               best_day_percent$day_of_week_mean[1] * 100
             ),
             "% IR"))
    best_day_percent
  })
  ...
  output$worst_day_1 <- renderText({
    ...
    worst_day_percent <-
      paste0(worst_day_percent$day_of_week[1],
             ":\n ",
             paste0(round(
               worst_day_percent$day_of_week_mean[1] * 100
             ),
             "% IR"))
    worst_day_percent
  })
  ...
  dailyData <- reactive({
    validate(need(!is.null(getData()), "No data uploaded yet"))
    validate(need(
      !is.null(event_data(event = "plotly_click", source = "date")),
  ...
  output$dailyGraph <- renderPlotly({
    ...
    if (mmol_or_dL() == "(mmol/L)") {
      plot <-
        plot + scale_y_continuous(limits = c(0, 25), name = "Sensor glucose (mmol/L)")
    } else {
      plot <-
        plot + scale_y_continuous(limits = c(0 * 18, 25 * 18), name = "Sensor glucose (mg/dL)")
    }
  ...
  cumAverage_df <- reactive({
    validate(need(!is.null(getData()), "No data uploaded yet"))
    validate(need(nrow(cgmData()) >= 50, paste0("Need at least 50 days of data. ", 
                                                "There are currently ", nrow(cgmData()), ".")))
    ...
    df <- cgmData() %>%
      ...
      drop_na() %>%
      mutate(in_range_cum_mean =  zoo::rollmean(percent_in_range,
                                                50,
                                                fill = NA,
                                                align = "right"))
    df
  ...
  output$cum_inrange <- renderPlotly({
    ...
    ymin <-
      max(0, (min(
        cumAverage_df()$in_range_cum_mean, na.rm = TRUE
      ) - 0.15), na.rm = TRUE)
    ymax <-
      min(1, (max(
        cumAverage_df()$in_range_cum_mean, na.rm = TRUE
    ...
    plot <-
      ...
      scale_y_continuous(
        ...
        name = "% readings in range (rolling average)",
        limits = c(ymin, ymax)
      )
  ...
  date_densityData <- reactive({
    if (input$date_comparison == TRUE) {
      df <-  getData() %>%
        mutate(date = as.Date(date_time, "%Y-%m-%d", tz = Sys.timezone())) %>%
        mutate(date_rangeIndicator = ifelse(
        ...
        mutate(low_event = ifelse(`Sensor Glucose` < input$target_range[1],
                                  1,
                                  0)) %>%
        mutate(high_event = ifelse(`Sensor Glucose` > input$target_range[2],
                                   1,
                                   0))
  ...
  dailyAverage_df <- reactive({
    ...
    if (input$date_comparison == FALSE) {
      df <- getData() %>%
        ...
        mutate(pLow = quantile(`Sensor Glucose`,
                               probs = 0.25,
                               na.rm = TRUE)) %>%
        mutate(pHigh = quantile(`Sensor Glucose`,
                                probs = 0.75,
                                na.rm = TRUE)) %>%
        distinct(Time_round, .keep_all = TRUE) %>%
        ungroup() %>%
    ...
    if (input$interquartile_range == TRUE) {
      if (input$date_comparison == FALSE) {
        dailyAverage_df() %>%
          ...
          layout(
            yaxis = list(
              title = paste0("Sensor glucose ", mmol_or_dL()),
              ticklen = 5,
              range = c(0, 20)
            ),
            xaxis = list(
  ...
  # Impact of meals ####
  ### #### #### #### #### #### #### #### #### #### #### #### #### #
  # Adjust the color comparison.
  
  
  # Look at impact 3 hours after meal
  meal_df <- reactive({
    validate(need(!is.null(getData()), "No data uploaded yet"))
    if (input$date_comparison == FALSE) {
    df_meal <- getData() %>%
      # Create data frame that consists of the blood glucose readings three hours after meal
      mutate(date = as.Date(date_time, "%Y-%m-%d", tz = Sys.timezone())) %>%
      filter(date >= as.Date(input$date1[1]) &
               date <= as.Date(input$date1[2])) %>%
      mutate(meal_group = ifelse(grepl('Meal', `Event Marker`), seq(1, 10000000, by =
                                                                         1), NA)) %>%
      fill(meal_group, .direction = "down") %>%
      # Extract the size of the meal and create meal size groups
      separate(`Event Marker`, into = c("Event Marker", "Carb size"), sep = ": ") %>% 
      mutate(`Carb size` = as.numeric(str_remove(`Carb size`, "\\..*"))) %>% 
      mutate(`Carb size group` = ifelse(`Carb size`<25, "Less than 25 grams",
                                        ifelse(`Carb size`>=25 & `Carb size`<=45,
                                               "Between 25 and 45 grams",
                                               ifelse(`Carb size`>45,
                                                      "Greater than 45 meals",
                                                      "")))) %>%    
      fill(`Carb size group`, .direction = "down") %>% 
      group_by(meal_group) %>%
      # By group, check if the time is within three hours of the first observations
      group_by(meal_group) %>%
      mutate(meal_time = first(date_time)) %>%
      mutate(meal_within_time_3 = if_else(
        difftime(date_time, meal_time, units = "hours") <= 3,
        1,
        0
      )) %>% 
      mutate(`Carb size group` = replace(`Carb size group`, meal_within_time_3==0, ""))          
    } else {
      df_meal <- getData() %>%
        # Create data frame that consists of the blood glucose readings three hours after meal
        mutate(date = as.Date(date_time, "%Y-%m-%d", tz = Sys.timezone())) %>%
  ...
  meal_df_elapse <- reactive({
    if (input$date_comparison == FALSE) {
    df <- meal_df() %>%
      filter(meal_within_time_3 == 1 &
               !is.na(meal_group) &
               is.na(`Event Marker`)) %>%
      arrange(date_time) %>%
      group_by(meal_group) %>%
      ...
      mutate(
        time_elapsed_meal = difftime(
          time_elapsed_round,
          group_first_time_round,
          units = "secs",
          tz = Sys.timezone()
        )
      ) %>%
      mutate(time_elapsed_meal = seconds_to_period(time_elapsed_meal)) %>%
      # Get average by 15 minutes, by meal group
      group_by(time_elapsed_meal) %>%
      mutate(glucose_mean = mean(`Sensor Glucose`, na.rm = TRUE)) %>%
      mutate(pLow = quantile(`Sensor Glucose`,
                             probs = 0.25,
                             na.rm = TRUE)) %>%
      mutate(pHigh = quantile(`Sensor Glucose`,
                              ...
                              na.rm = TRUE)) %>%
      ungroup() %>%
      distinct(time_elapsed_meal, .keep_all = TRUE) %>%
      # Need to change time elapsed to factor
      arrange(time_elapsed_meal) %>%
      mutate(time_elapsed_meal = as.character(time_elapsed_meal)) %>%
      mutate(time_elapsed_meal = sub("^(\\S*\\s+\\S+).*", "\\1", time_elapsed_meal)) 
    } else {
      df <- meal_df() %>%
        filter(meal_within_time_3 == 1 &
                 !is.na(meal_group) &
        ...
        mutate(time_elapsed_meal = seconds_to_period(time_elapsed_meal)) %>%
        # Get average by 15 minutes, by meal group
        group_by(time_elapsed_meal, date_rangeIndicator) %>%
        mutate(glucose_mean = mean(`Sensor Glucose`, na.rm = TRUE)) %>%
        mutate(pLow = quantile(`Sensor Glucose`,
                               probs = 0.25,
                               na.rm = TRUE)) %>%
        mutate(pHigh = quantile(`Sensor Glucose`,
                                ...
                                na.rm = TRUE)) %>%
        ungroup() %>%
        distinct(time_elapsed_meal, date_rangeIndicator, .keep_all = TRUE) %>%
        # Need to change time elapsed to factor
        arrange(time_elapsed_meal) %>%
        mutate(time_elapsed_meal = as.character(time_elapsed_meal)) %>%
        mutate(time_elapsed_meal = sub("^(\\S*\\s+\\S+).*", "\\1", time_elapsed_meal))
    ...
    levels_df <- df %>%
      distinct(time_elapsed_meal) %>% 
      pull(time_elapsed_meal)
    
    df$time_elapsed_meal <-
      factor(df$time_elapsed_meal, levels = levels_df)
  ...
  output$average_meal_elapse <- renderPlotly({
    validate(need(!is.null(getData()), "No data uploaded yet"))
    if (input$date_comparison == FALSE) {
    if (input$interquartile_range == FALSE) {
      ...
      date_range1 <- meal_df_elapse() %>% 
        filter(date_rangeIndicator=="Date range 1")
      
      date_range2 <- meal_df_elapse() %>% 
        filter(date_rangeIndicator=="Date range 2")
      ...
      if (input$interquartile_range == FALSE) {
        ...
        plot_ly() %>%
          ...
          add_ribbons(
            ...
            line = list(color = 'rgba(0, 0, 225, 0.05)'),
            fillcolor = 'rgba(0, 0, 225, 0.3)',
            name = 'Middle 50% of readings per time after meal'
          ) %>%
          layout(
            yaxis = list(
              title = paste0("Sensor glucose ", mmol_or_dL()),
  ...
  exercise_df_24_elapse <- reactive({
    df <- exercise_df() %>%
      ...
      mutate(group_first_time_round = (round_date(group_first_time, "15 minutes"))) %>%
      mutate(
        time_elapsed_exercise = difftime(
          time_elapsed_round,
          group_first_time_round,
          units = "secs",
          tz = Sys.timezone()
    ...
    df$time_elapsed_exercise <-
      factor(df$time_elapsed_exercise, levels = levels_df)
  ...
  output$downloadData <- downloadHandler(
    ...
    content = function(file) {
      write.csv(date1Data(), file, row.names = FALSE)
    }
  )
